  ...
  Lwt_main.run
    (let* conf_with_ca_detect = get_ethereal_account_details () in
     let conf_with_ca_cert_bundle =
       Config.set_ca_cert "/etc/ssl/certs/ca-certificates.crt" conf_with_ca_detect
     in
     let conf_with_single_ca_cert =
       ...
       Config.set_ca_cert "/etc/ssl/certs/DST_Root_CA_X3.pem" conf_with_ca_detect
     in
     let conf_with_ca_path = Config.set_ca_path "/etc/ssl/certs/" conf_with_ca_detect in
     Alcotest_lwt.run
       "STMP client"
